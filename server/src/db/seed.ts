...
async function seed() {
  try {
    console.log('Seeding database with initial bar data...');
    ...
    for (const bar of bars) {
      await query(
        ...
        [bar.id, bar.name, bar.nameEn, bar.districtId, bar.address, bar.image, bar.rating, bar.drinks]
      );
    }

    console.log(`✓ Seeded ${bars.length} bars successfully`);
    process.exit(0);
  } catch (error) {
