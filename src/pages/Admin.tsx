      ...
      <div className="flex flex-col gap-2">
        <div className="flex md:hidden items-center justify-between">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-dark-800 border border-gray-800 text-sm"
          >
            <span className="text-lg">☰</span>
            <span>{tabs.find(t => t.id === activeTab)?.label || '選單'}</span>
          </button>
          <button
            ...
            {tabs.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  activeTab === id ? 'bg-primary-500/20 text-primary-200 border border-primary-500/40' : 'bg-dark-800 text-gray-300 border border-gray-800 hover:border-gray-700'
                }`}
              >
